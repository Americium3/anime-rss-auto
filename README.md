...
| **web UI** | local dashboard: all bgm-marked shows grouped by collection type (在看/想看/看过/搁置/抛弃) with a type filter, card outlines glow in their collection type's color on hover, card titles follow the UI language (English/romaji titles via AniList in the English UI), every list sorted by premiere time (earliest first), live output of a manual sync in the log panel, a weekly timetable tab (watching + plan-to-watch shows with airing/upcoming status badges and a premiere-status filter, one column per weekday starting from today, today's column highlighted, airing slots in **your** local timezone), click any show card to open its bangumi page, dark/light theme with follow-system default, premiere banners stay for a week unless dismissed, per-show premiere time in **your** local timezone (via AniList), premiere banners, grace countdowns, switch subtitle group, per-show "n/m episodes ready to watch" summary, color-coded season badges (8 colors cycling every 2 years), manual sync, logs | `webui.py` |
