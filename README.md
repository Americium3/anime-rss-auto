...
| **prefer-variant** | some groups publish one episode in several variants — by source (Baha / CR / ABEMA / B-Global…) or by subtitle language (简日双语 / 繁日双语, also written JPSC / JPTC / CHS / CHT). Blacklisted sources (ABEMA, B-Global) are folded into every rule's `mustNotContain` so the feed rejects them and they never download; if more than one variant of the same episode does land, only the highest-priority one is kept after download and the rest are deleted (files included). Ranking is lexicographic across dimensions (source first, then language: Baha ＞ CR, simplified ＞ traditional). The language tier is a set of synonym markers, so both CJK (简/繁) and Latin abbreviations (SC/TC/CHS/CHT/GB/BIG5, incl. glued forms like JPSC/JPTC) are recognized while mid-word false hits (disc/watch) are avoided. Untagged releases are left untouched, and only cours after the cutoff are affected. Run standalone with `python anime_rss.py dedup [--dry-run]` | `prefer_variant_enabled` / `source_blacklist` / `source_priority` / `lang_priority` |
...
| **season cutoff** | shows older than a cour cutoff are never touched — no adds, no deletes. A cross-cour show (半年番/年番) keeps being auto-managed while it is still broadcasting: its span in cours is inferred from bgm's total episode count (`⌈eps/13⌉`), and once that span elapses it reverts to a manual old show — so a still-airing 2-cour/year-long show is not frozen just because its start cour fell behind the cutoff. Individual shows can also be pinned "current" by bgm id | `skip_before_season` / `pin_current_bgm_ids` |
...
- Destructive actions (deleting files/rules) only ever apply to shows from
  ...
  (an explicit per-show override), and a cross-cour show still inside its
  broadcast span (a 半年番/年番 whose `⌈total_eps/13⌉` cours have not yet elapsed
  since it started). A finished long show past its span is read-only again.
