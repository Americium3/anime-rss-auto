...
| **manual resolve** | the "no mikan match" banner carries a paste box: drop a mikan link in and the show is bound by hand, no `mikan_overrides.json` editing. A `Bangumi` page link — or an `Episode` page link whose page names its bangumi — persists the override and subscribes through the normal pipeline (feed + rule + mikan sub, same folder conventions); an `Episode` link is also downloaded itself immediately (the exact release you picked, even if the rule's group filter would never match it). An `Episode` page with **no** bangumi backlink (one-shot releases: 冷番补完-style movies/specials that belong to no mikan bangumi and can never match RSS) or a raw magnet URI becomes a one-shot import: the magnet is handed straight to qB under the show's library folder, and `manual_imports.json` records `infohash → bgm subject` so mark-watched / jfhook / autocomplete claim the ruleless torrent exactly as if a rule had downloaded it (a subject with a single main episode resolves without an episode number in the file name — movie treatment). A 想看 show is promoted to 在看 on success, and the banner clears immediately. Also scriptable: `python anime_rss.py resolve --bgm-id N --url <link>` | always on |
| **uncatalogued stock** | the banner above is dismissible and the state it reports is not, so acknowledging the warning used to take the only control that could fix the show with it — the show still had no feed, the daemon still said so every pass, and nothing on the panel could reach it any more. A show mikan has no entry for is now marked on the wall itself: its cover stands drained of colour (returned on hover, so it can still be identified) with a ruled slip of paper tucked into its fore-edge, an **Awaiting mikan** chip counts them across every collection type at once — a film sits in 想看, not 在看, which is why this cuts across the tabs instead of being one of them — and the show's own card carries the two ways in as separate controls: bind a mikan entry (subscribes the season and follows it from then on) or paste a magnet (takes that one release into the library). Both reach the same endpoint, which classifies whatever is pasted, so neither can reject what belongs in the other; they stay apart because they are different decisions, and a film — most of what lands here, mikan being worst at exactly those — is nearly always the second. None of these can be dismissed: they clear when the show resolves, and not before | always on |
...
| **no-raw guard** | a fallback subtitle group (not in `group_priority`, so no custom filter) would otherwise get an empty `mustContain` and grab anything in its mikan feed — including cross-posted streaming raws with no Chinese subtitle (e.g. a Netflix dual-audio rip carrying the show's English title). Two lines of defence: new rules for such groups require a Chinese-subtitle marker in the title (`cjk_sub_required`, a single `\|`-OR term so the feed rejects raw releases — `CR` and `Baha` are whitelisted too, since Crunchyroll and Bahamut 動畫瘋 rips carry official Chinese subs but tag the title with the platform, not the language, so groups that only mirror those platforms aren't blocked wholesale; the untranslated ABEMA/B-Global rips stay excluded via `source_blacklist`), and any downloaded torrent whose name matches a raw-platform tag (`hard_reject_tags`, e.g. `NF WEB-DL`) is deleted outright regardless of siblings. When the whitelist grows, existing fallback rules are upgraded in place on the next pass (old default strings are recognized by prefix, custom per-group filters are never touched). Both respect the season cutoff, and both stand down for a show subscribed to mikan's raw bucket (`raw_subgroup_ids` — mikan files every untagged upload under one global subgroup, id 202, and calls it 「生肉/不明字幕」): pointing a rule there is a standing request for exactly what these two throw away, and enforcing them means downloading the asked-for episode and deleting it again every pass, forever, saying so only in the log | `cjk_sub_required` / `hard_reject_tags` / `raw_subgroup_ids` |
| **mikan re-file** | a show whose mikan entry doesn't exist yet falls to the loosest search tier and binds to the nearest title — for a cour bgm split in two, that is the *other* half. The rule built from that guess then vouches for it, because the resolve fast path trusts any mapping a live feed carries, so the guess outlives its TTL forever; when mikan later opens the show's own entry, the old feed simply stops mid-season and nothing errors. Unconfirmed (`low`) mappings are therefore re-searched on the normal 24h TTL even while a rule carries them, and when one moves the rule follows it: same folder, same subtitle group, same `previouslyMatchedEpisodes` (episodes already downloaded must not come back), only the `bangumiId` changes, and the abandoned feed is dropped once nothing points at it. A feed claimed by two watched subjects at once is left alone — moving it would unsubscribe the other half. Confirmed mappings (the mikan page names the bgm subject) cannot move and stay network-free | always on |
...
| **split-cour continuation** | a fansub numbers a season straight through while bgm splits that season into two subjects, the second restarting at episode 1 — so ANi's `第四季 - 13` matches nothing: not the first subject, which stopped at 11, and not the second, whose own numbering calls it 2 and whose whole-series `sort` calls it 79. The episode reaches neither qB's stop nor bangumi, and the only trace is one skip line. When a number fits neither numbering of the subject the mikan page names, the episode is retried against the other subject that resolved onto the same mikan entry, offset by however many main episodes the first one had. Applied only when all of: both subjects resolved onto the same feed; the number is not already valid in the earlier subject (they overlap — both have an episode 5); it becomes valid in the later one once offset, at 1 or above; and the earlier subject finished broadcasting before the later one began — which also settles direction. When the mikan page names the **later** half instead, no sibling can answer for the number (that entry resolves to the later subject and nothing else), so the offset comes from the other side: bangumi's own `前传` relation names the earlier half, and the episode then belongs to this very subject shifted down by however long that half ran. That is the shape mikan leaves once it gives a split cour two entries, and the only shape a show first resolved *after* the split ever has | always on |
...
| **jf-played reconcile** | the pull half of the Jellyfin link, and the reason a missed episode is no longer permanent. jfhook is *at-most-once*: Jellyfin fires a webhook once and never replays it, so an episode watched during any window where the hook could not act on it — a resolver bug, a daemon restart, the plugin failing to deliver — was lost with nothing left to retry. mark-watched cannot cover for it either, because it waits on a seeding → stopped transition and a hook that failed never stopped the torrent, so the transition it waits for is exactly the one that never comes: both halves of the design depended on the same delivery. This pass sweeps Jellyfin's `Played` flag every run and marks anything bgm has no record of, stopping the torrent the failed hook left seeding. A ledger of item ids keeps a steady pass at one Jellyfin call and no bgm calls, and **which failures get written to it is the whole design**: settled verdicts (Ancient, an old cour, no rule, torrent gone) are recorded and never revisited, while anything that a later fix could change — bgm not answering, or an episode number this code does not yet understand — is deliberately left out, so the sweep comes back for it instead of having certified the bug's answer as final. A first run records the existing backlog without acting on it. Run standalone with `python anime_rss.py jf-played [--dry-run]` | `jellyfin_played_reconcile_enabled` / `jellyfin_played_scan_limit` |
...
- `anime_rss.py` — everything above except the panel; stdlib only, single file.
  Subcommands: `list`, `plan`, `apply`, `prune`, `sync`, `watch`, `mark`, `autocomplete`, `resolve`, `dedup`, `premiere`, `analyze`, `auth`, `jfhook`, `jf-played`.
...
`jellyfin_url` defaults to `http://127.0.0.1:8096`, and the literal address
matters: on Windows, `localhost` resolves to `::1` first, and a Jellyfin bound
only to IPv4 leaves that connection to time out before the retry — a flat ~2s on
*every* call, measured here at 2.05s against 0.016s. If your config still names
`localhost`, that tax is most of what the Jellyfin passes cost.
