...
| **season cutoff** | shows older than a cour cutoff are never touched — no adds, no deletes. Individual long-running old shows (a weekly short that keeps airing) can be pinned back to "current" by bgm id, so they are auto-managed everywhere despite their date | `skip_before_season` / `pin_current_bgm_ids` |
...
- Destructive actions (deleting files/rules) only ever apply to shows from
  ...
  unless a bgm id is listed in `pin_current_bgm_ids` (an opt-in per-show
  override that makes an old-but-still-airing show behave like a current one).
