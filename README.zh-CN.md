...
| **Web 控制面板** | 本地仪表盘：每部番都是一本立在自己小挂架上的精装书——封面是书的正面，书脊带书顶帽、缝线书带、烫印标签栏，有的还压了徽章；布面、烫箔与这些装帧细节都由该番自己的 ID 决定，没有两本装帧相同。悬停时书正过来并抬离搁板，点击则在屏幕正中弹出这部番的完整卡片。窗口宽于 1800px 时，页面不再是一根居中的栏，而是一间屋子：左墙是藏书阁——每部看过的番以书脊侧立，布面与烫箔仍取自它自己的装帧，番名竖排刻在脊上，脊底一条按 bangumi 评分着色的尾带，可按季度／评分／年代重新上架（支持深链 `?sort=score`），点一下同样弹出那张卡片；脚下每个字幕组一枚歪着盖的印章，标着这面墙上有多少卷出自该组。右墙是值班台：本地与东京双钟之下是接下来 48 小时的放送（AniList 给的是第一集时刻，故按周向前推算，且只推算后端仍判定为在播的番，已完结的番绝不会被凭空安排一场放送）、一本横格登记簿记录最近入库的剧集（当天的盖章）、以及一块机器状态板（qBittorrent 是否在线、上次同步、订阅数，以及 qBittorrent 里每集一格的计数条）。每条侧栏顶上一盏壁灯往墙面打下光锥，立柱与踢脚线框住整间屋子，滚动时家具相对结构缓缓错位，而壁灯与立柱钉在墙上不动。窄于 1800px 时这一切根本不会构建，布局与原先完全一致。所有 bgm 标记过的番按收藏类型分组（在看/想看/看过/搁置/抛弃）并可按类型筛选 + 番名实时搜索框；主题、语言、字幕组优先级与守护进程日志都收进设置齿轮，不再裸露在页面上。每张卡片标题旁带 bangumi 与 mikan 两个 favicon 按钮、链接到该番对应页面（mikan 已知 feed 时直达番组页，否则退回 mikan 番名搜索），番名跟随界面语言（英文界面经 AniList 显示英文/罗马字名）、所有列表（时刻表除外）按季度分块、每个季度有整行彩色分割线、季度未知的沉到最后——在看与想看按季度从旧到新排（当作待清的积压），看过／搁置／抛弃按从新到旧排；块内看过页按刚标记看过的排前面（读作观看历史）、其余标签页按开播时间从早到晚排序、手动同步时日志面板实时显示同步输出（结束后仍可回看；失败会弹提示并自动展开日志）、周内时刻表页（含在看+想看、开播状态用时刻颜色区分、可按开播状态筛选、另有「仅本季」开关把面板限定为当前季度——被钉为当前、以及仍在播的跨季番视作当前、不会被藏、每个周几一根竖列、从今天开始往后排、今天整列高亮、可抓住面板空白处左右拖动、按**本地时区**播出时刻排列、每张卡还带完整本地化的开播日期/时刻）、在看网格每张卡显示该番每周更新时段（周几+本地时刻），并带一个「按更新星期几」子筛选（每个周几一枚带实时计数的药丸，可只看某一天更新的番）、每部跨季番卡片带「半年番/年番」角标（按放送日程跨度判定——首集到末集的 airdate 区间，而非集数）、深色/浅色配色（默认跟随系统）、开播横幅保留一周（可手动关闭，多条时有「全部知道了」按钮）、「未匹配 mikan」警告横幅（已开播却没能解析到 feed 的番，持续到番解析成功或你手动关闭，多条时有「全部知道了」按钮）、每部番显示**你本地时区**的开播时间（经 AniList）、宽限倒计时、切换字幕组（会删除该番旧字幕组已下载的文件、再从新组逐集重下整季——不可撤销，受季度分界线保护并有确认弹窗）、每番「n/m 集准备完成」汇总、季度彩色徽章（8 色两年一循环）、每卡 bangumi 社区评分徽章（颜色随分数从红——最低——渐变到绿——最高）、所选标签页记忆并支持深链（`?tab=schedule&theme=light&lang=zh`）、全屏开场序列（隔板自绘、精装书逐本立上架、字标逐字落位，真实数据到位的一刻幕布上卷揭开页面；没有假进度条；后端未就绪时书变琥珀色并每 5 秒快速重试；`?boothold` 可冻结用于截图）、所有时间戳本地化、页头离线 / qBittorrent 掉线 / AniList 无响应指示（开播时间取自 AniList，它不响应时时刻表会明说，而不是把所有番默默塞进「时间待定」）、手机友好布局与触控目标、键盘/读屏可用、全站克制的动效（卡片瀑布式逐张入场与启动页揭开时的入场编排、背景光斑缓慢漂移呼吸、时刻表今天列标出当前时刻的脉冲线、主题切换交叉淡入及悬停/按压微交互——入场动画只在用户操作时播放，后台轮询绝不重播）、遵循系统减少动效设置（支持中英切换） | `webui.py` |
