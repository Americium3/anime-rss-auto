...
| **防生肉误抓** | 兜底字幕组（不在 `group_priority`、无自定义过滤词）原本 `mustContain` 为空 = feed 里啥都收，会抓到 mikan 交叉发布的无中文字幕流媒体生肉（如带该番英文译名的 Netflix 双语版）。两道防线：这类组的新规则要求标题含中文字幕标记（`cjk_sub_required`，单个 `\|`-或串，feed 层拒生肉——`CR` 也在白名单内，因为 Crunchyroll 版内封官方简繁中字、标题只标平台不标语言，避免「只搬运 CR/ABEMA」的番被整条误挡；无中文翻译的 ABEMA/B-Global 仍由 `source_blacklist` 排除），且任何下载到的、名字命中生肉平台标记（`hard_reject_tags`，如 `NF WEB-DL`）的种子无条件删除、不受「唯一版本」保护。两者都守季度分界红线 | `cjk_sub_required` / `hard_reject_tags` |
