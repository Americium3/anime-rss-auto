...
| **Web 控制面板** | 本地仪表盘：按收藏类型分组显示所有 bgm 标记过的番（在看/想看/看过/搁置/抛弃）并可按类型筛选、光标悬停时卡片轮廓发出对应收藏类型颜色的光晕、番名跟随界面语言（英文界面经 AniList 显示英文/罗马字名）、所有列表按开播时间从早到晚排序、手动同步时日志面板实时显示同步输出、周内时刻表页（含在看+想看、每张卡标已开播/未开播、可按开播状态筛选、每个周几一根竖列、从今天开始往后排、今天整列高亮、按**本地时区**播出时刻排列）、点击任意番剧卡片跳转对应 bangumi 条目页、深色/浅色配色（默认跟随系统）、开播横幅保留一周（可手动关闭）、每部番显示**你本地时区**的开播时间（经 AniList）、开播横幅、宽限倒计时、切换字幕组、每番「n/m 集准备完成」汇总、季度彩色徽章（8 色两年一循环）、手动同步、日志（支持中英切换） | `webui.py` |
