...
| **旧番分界线** | 早于分界季度的番永不触碰——不增、不删。跨季番（半年番/年番）只要还在播就继续自动管理：按 bgm 总集数推算跨度（`⌈集数/13⌉` 季），跨度用完才回退成手动老番——所以还在更新的 2-cour/年番不会因为开播季度落在分界线之前就被冻结。个别番也可按 bgm id 手动钉为「当前」 | `skip_before_season` / `pin_current_bgm_ids` |
...
- `anime_rss.py` —— 除面板外的全部功能；纯标准库，单文件。
  子命令：`list`、`plan`、`apply`、`prune`、`sync`、`watch`、`mark`、`dedup`、`premiere`、`auth`、`jfhook`。
...
- 破坏性操作（删文件/删规则）只对 `skip_before_season` 及之后的番生效；
  ...
  `pin_current_bgm_ids`（手动逐番开启），以及仍在播出跨度内的跨季番
  （半年番/年番，自开播起 `⌈总集数/13⌉` 季尚未走完）。跨度走完的完结长番
  重新变回只读。
