...
| **旧番分界线** | 早于分界季度的番永不触碰——不增、不删。个别持续更新的老番（如每周更新的短番）可按 bgm id 钉回「当前季」，从而无视日期、在所有环节都当当前番自动管理 | `skip_before_season` / `pin_current_bgm_ids` |
...
- 破坏性操作（删文件/删规则）只对 `skip_before_season` 及之后的番生效；
  更早的番对本工具严格只读——除非其 bgm id 列入 `pin_current_bgm_ids`
  （逐番手动开启的例外，把仍在更新的老番当当前番处理）。
